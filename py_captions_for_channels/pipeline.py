...
class Pipeline:
    ...
    # Signals that indicate the process was killed externally (not a code-level exit)
    _SIGNAL_EXIT_CODES = {
        134: "SIGABRT",
        137: "SIGKILL",
        139: "SIGSEGV",
    ...
    def _validate_crash_recovery(self, recording_path: str, log) -> bool:
        ...
        the pipeline may have already written the output file before the
        crash occurred. This validates the output by checking for our
        subtitle track marker embedded by embed_captions.

        Returns True if the output file contains a valid
        'py-captions-for-channels' subtitle track.
        """
        try:
            result = subprocess.run(
                [
                    ...
                    "s",
                    "-show_entries",
                    "stream=codec_name:stream_tags=handler_name,title",
                    "-of",
                    "json",
                    recording_path,
                ],
            ...
            for stream in streams:
                ...
                if (
                    "py-captions-for-channels" in handler
                    or "py-captions-for-channels" in title
                ):
                    log.info(
                        "Crash recovery: output file has valid "
                        "'py-captions-for-channels' subtitle track"
                    )
                    return True
            log.debug(
                "Crash recovery: no py-captions-for-channels "
                "subtitle track found in %s",
                recording_path,
            )
            return False
        except Exception as e:
            log.debug("Crash recovery validation failed: %s", e)
            return False
        ...
        try:
            ...
            try:
                ...
                if returncode != 0:
                    ...
                    # --- Crash recovery for signal-based exits ---
                    # When a process is killed by a signal (exit > 128),
                    # the work may have completed before the crash
                    # (e.g., SIGSEGV during NVIDIA driver cleanup).
                    # Validate the output file before declaring failure.
                    if returncode > 128:
                        ...
                        if self._validate_crash_recovery(event.path, log):
                            log.warning(
                                ...
                                "treating as success "
                                "(crash likely during process shutdown)",
                                signal_name,
                                returncode,
                                elapsed,
                            )
                            ...
                            log.info("=" * 80)
                            log.info(
                                f"END {job_marker}: RECOVERED "
                                f"({signal_name}) in {elapsed:.1f}s"
                            )
                            log.info("=" * 80)
                            return PipelineResult(
                                success=True,
