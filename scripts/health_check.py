...
# Regex to extract successful pipeline completions
...
def parse_log(log_path: Path, start_offset: int) -> Tuple[Set[Path], int]:
...
def main() -> None:
    ...
    completed_paths, new_offset = parse_log(log_path, start_offset)
    LOG.info("Found %d completed recordings in new log entries", len(completed_paths))
